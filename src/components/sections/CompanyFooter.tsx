...
export function CompanyFooter({ companyName, companyColor, tagline, email, phone }: CompanyFooterProps) {
  return (
    <footer id="contact" style={{ background:"#040A14",borderTop:`1px solid ${companyColor}22` }}>
      {/* CTA band */}
      <div style={{ background:`linear-gradient(135deg,#0A1628,#112240)`,borderBottom:`1px solid ${companyColor}1A`,padding:"72px 64px" }}>
        <div style={{ maxWidth:1100,margin:"0 auto",display:"flex",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:32 }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            <a href={`mailto:${email}`}
              style={{ background:`linear-gradient(135deg,${companyColor}bb,${companyColor})`,color:"#0A1628",fontSize:12,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",padding:"14px 32px",borderRadius:2,textDecoration:"none",display:"flex",alignItems:"center",gap:8,boxShadow:`0 8px 24px ${companyColor}33` }}>
      ...
      <div style={{ padding:"28px 64px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12 }}>
        <div style={{ display:"flex",alignItems:"center",gap:12 }}>
          ...
          <span style={{ fontSize:10,color:"rgba(255,255,255,0.25)",letterSpacing:"0.1em" }}>· {tagline}</span>
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:20 }}>
          <Link href="/" style={{ fontSize:11,color:"rgba(255,255,255,0.3)",textDecoration:"none",display:"flex",alignItems:"center",gap:5,transition:"color 0.3s" }}
            onMouseEnter={e=>(e.currentTarget.style.color=companyColor)}
            onMouseLeave={e=>(e.currentTarget.style.color="rgba(255,255,255,0.3)")}
          >
            ← Back to UnifiedNexus Group
          </Link>
          <span style={{ fontSize:10,color:"rgba(255,255,255,0.18)" }}>© {new Date().getFullYear()} UnifiedNexus Group</span>
        </div>
      </div>
    </footer>
  );
