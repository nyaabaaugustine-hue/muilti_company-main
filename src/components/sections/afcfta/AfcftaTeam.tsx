...
export function AfcftaTeam() {
  const A="#F59E0B";
  return (
    <section id="team" style={{background:"#1E293B",padding:"clamp(40px, 8vw, 100px) clamp(16px, 4vw, 64px)"}}>
      <style>{`
        .afcfta-team-grid{display:grid;grid-template-columns:1fr;gap:16px;}
        @media(min-width:640px){
          .afcfta-team-grid{grid-template-columns:repeat(auto-fill,minmax(200px,1fr));}
        }
      ...
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(28px,5vw,48px)",fontWeight:600,color:"#fff",marginBottom:52}}>
          The Team <em style={{color:A,fontStyle:"italic",fontWeight:400}}>Driving Change</em>
        </h2>
        <div className="afcfta-team-grid">
          {TEAM.map((m,i)=>(
            <div key={i}
