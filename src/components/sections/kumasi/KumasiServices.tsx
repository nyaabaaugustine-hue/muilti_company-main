...
export function KumasiServices() {
  ...
  return (
    <section id="services" style={{ background:"#141008",padding:"100px 64px" }}>
      <div style={{ maxWidth:1100,margin:"0 auto" }}>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:56 }}>
          ...
          <p style={{ fontSize:13,color:"rgba(255,255,255,0.45)",maxWidth:300,lineHeight:1.75,fontWeight:300 }}>Every development is a statement of intent — bold, smart, and unmistakably Ghanaian.</p>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16 }}>
          {S.map((s,i)=>{ const Icon=s.icon; return (
            <div key={i} onMouseEnter={()=>setHov(i)} onMouseLeave={()=>setHov(null)}
              style={{ background:hov===i?"rgba(201,168,76,0.07)":"rgba(22,17,6,0.8)",border:`1px solid ${hov===i?"rgba(201,168,76,0.4)":"rgba(201,168,76,0.1)"}`,borderRadius:4,padding:"28px 24px",transition:"all 0.4s cubic-bezier(0.22,1,0.36,1)",transform:hov===i?"translateY(-6px)":"translateY(0)" }}>
              <div style={{ width:44,height:44,background:"rgba(201,168,76,0.12)",border:"1px solid rgba(201,168,76,0.25)",borderRadius:2,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:18 }}>
