  ...
  return (
    <section id="contact" className="wan-contact" style={{ background: "#0A0A0A", padding: "clamp(40px, 8vw, 100px) clamp(16px, 4vw, 64px)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div className="wan-contact-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 48, alignItems: "start" }}>
          {/* Left info */}
          <div>
          ...
          {/* Right form */}
          <div style={{ background: "#1A1A1A", border: `1px solid ${GOLD}33`, borderRadius: 4, padding: "clamp(20px, 4vw, 36px)", position: "relative", overflow: "hidden" }}>
            ...
            {sent ? (
              <div style={{ textAlign: "center", padding: "40px 0" }}>
                ...
                <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(24px, 4vw, 28px)", color: "#fff", marginBottom: 10 }}>Message Received!</h3>
                <p style={{ fontSize: "clamp(13px, 2.5vw, 14px)", color: "rgba(255,255,255,0.60)" }}>We'll respond within 24 hours.</p>
                <button onClick={() => setSent(false)} style={{ marginTop: 20, background: GOLD, color: "#000", border: "none", padding: "10px 24px", borderRadius: 2, cursor: "pointer", fontSize: "clamp(11px, 2vw, 12px)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                  Send Another
                </button>
              </div>
            ) : (
              <form onSubmit={handle} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input type={type} placeholder={placeholder} value={form[key as keyof typeof form]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} required
                      style={{ width: "100%", padding: "11px 14px", background: "#0A0A0A", border: `1px solid ${GOLD}33`, borderRadius: 3, fontSize: "clamp(13px, 2.5vw, 16px)", color: "#fff", outline: "none", fontFamily: "inherit", transition: "border-color 0.2s", boxSizing: "border-box" }}
                ...
                <div>
                  ...
                  <textarea placeholder="How can we assist you?" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} required rows={4}
                    style={{ width: "100%", padding: "11px 14px", background: "#0A0A0A", border: `1px solid ${GOLD}33`, borderRadius: 3, fontSize: "clamp(13px, 2.5vw, 16px)", color: "#fff", outline: "none", fontFamily: "inherit", resize: "vertical", transition: "border-color 0.2s", boxSizing: "border-box" }}
                    onFocus={e => (e.target.style.borderColor = GOLD)}
                    onBlur={e => (e.target.style.borderColor = `${GOLD}33`)} />
                </div>
                <button type="submit"
                  style={{ background: GOLD, color: "#000", border: "none", padding: "13px 28px", borderRadius: 2, cursor: "pointer", fontSize: "clamp(11px, 2vw, 12px)", fontWeight: 700, letterSpacing: "0.10em", textTransform: "uppercase", alignSelf: "flex-start" }}>
                  Send Message →
                </button>
              </form>
            )}
