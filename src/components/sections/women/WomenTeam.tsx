...
export function WomenTeam() {
  return (
    <section id="team" style={{ background: "#fff", padding: "100px 64px" }}>
          ...
          {TEAM.map((m, i) => (
            <div key={i}
              ...
              <div style={{ padding: "16px 18px 20px", borderTop: `1px solid ${GOLD}18` }}>
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 700, color: "#0A0A0A", marginBottom: 5, lineHeight: 1.2 }}>{m.name}</div>
                <div style={{ fontSize: 11, color: GOLD, fontWeight: 700, letterSpacing: "0.05em" }}>{m.role}</div>
              </div>
            </div>
          ))}
        </div>
